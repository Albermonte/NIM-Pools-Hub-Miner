...
export default class NativeMiner extends Nimiq.Observable {
  ...
  constructor(type, deviceOptions) {
    ...
    const threads = this._devices.reduce(
      (threads, device) => threads + (device.enabled ? device.threads : 0),
      4
    ); // 4 initial threads + more for GPU workers
  ...
  _reportHashRate() {
    ...
    this._hashes.forEach((hashes, idx) => {
      ...
      if (
        this._lastHashRates[idx].length > NativeMiner.HASHRATE_MOVING_AVERAGE
      ) {
        this._lastHashRates[idx].shift();
        averageHashRates[idx] =
          this._lastHashRates[idx].reduce((sum, val) => sum + val, 0) /
          this._lastHashRates[idx].length;
  ...
  startMiningOnBlock(blockHeader, callback) {
    if (!this._hashRateTimer) {
      this._hashRateTimer = setInterval(
        ...
        1000 * NativeMiner.HASHRATE_REPORT_INTERVAL
      );
    }
    this._nativeMiner.startMiningOnBlock(
      ...
      (error, obj) => {
        ...
        this._hashes[obj.device] =
          (this._hashes[obj.device] || 0) + obj.noncesPerRun;
        callback(obj);
      }
    );
  }
