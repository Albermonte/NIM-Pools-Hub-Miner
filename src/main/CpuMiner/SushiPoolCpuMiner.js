...
export default class SushiPoolCpuMiner extends Nimiq.Observable {
  constructor(address, deviceData, threads) {
    ...
    this._miner._reportHashRate = () => {
      ...
      const averageHashRate =
        this._lastHashRates.reduce((sum, val) => sum + val, 0) /
        this._lastHashRates.length;
    ...
    this._miner.setShareCompact = (shareCompact) => {
      if (this._miner._miningEnabled) {
        Nimiq.Log.i(
          SushiPoolCpuMiner,
          `Still working on block #${this._block}`
        );
  ...
  connect(host, port) {
    ...
    this._ws.on("close", (code, reason) => {
      ...
      if (!this._closed) {
        ...
        Nimiq.Log.w(
          SushiPoolCpuMiner,
          `Connection lost. Reconnecting in ${timeout} seconds to ${this._host}`
        );
        setTimeout(() => {
    ...
    this._ws.on("error", (e) =>
      Nimiq.Log.e(SushiPoolCpuMiner, `WS error - ${e.message}`, e)
    );
  }
  ...
  _register() {
    Nimiq.Log.i(
      SushiPoolCpuMiner,
      `Registering to pool (${this._host}) using device id ${this._deviceId} (${this._deviceData.deviceName}) as a dumb client.`
    );
    this._send({
      message: "register",
      mode: "dumb",
  ...
  _startMining() {
    ...
    const header = Nimiq.BlockHeader.unserialize(
      new Nimiq.SerialBuffer(this._currentBlockHeader)
    );
  ...
  _onBalance(balance, confirmedBalance) {
    Nimiq.Log.i(
      SushiPoolCpuMiner,
      `Balance: ${Nimiq.Policy.lunasToCoins(
        balance
      )} NIM, confirmed balance: ${Nimiq.Policy.lunasToCoins(
        confirmedBalance
      )} NIM`
    );
  ...
  _onNewBlock(blockHeader) {
    ...
    if (
      this._currentBlockHeader !== undefined &&
      this._currentBlockHeader.equals(blockHeader)
    ) {
